...
def main():
    ...
    if args.plot:
        ...
        # initialPose = gtsam.utilities.extractPose2(graph)
        # values = gtsam.Values()
        # for i in range(resultPoses.shape[0]):
        #     values.insert(i, gtsam.Pose2(resultPoses[i, :]))
            # plot.plot_pose2(1, gtsam.Pose2(resultPoses[i, :]))
        # plot.plot_trajectory(1, values)
        # plt.plot(initialPose[:, 0], initialPose[:, 1])
